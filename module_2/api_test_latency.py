...
logger.info(f"Testing {len(test_cases)} cases against {url}")
...
for i, case in enumerate(test_cases, 1):
    ...
    logger.info(f"[{i:2d}/{len(test_cases)}] {case['flow_type'].upper():7} | {case['description']}")
    ...
    try:
        ...
        logger.info(f"  Status: {response.status_code} | Latency: {latency:.3f}s")
        ...
        if response.status_code == 200:
            try:
                ...
                if isinstance(result, dict):
                    ...
                    output = str(result)
                output_snippet = str(output)[:150] + ('...' if len(str(output)) > 150 else '')
                logger.info(f"  Output: {output_snippet}")
            except requests.exceptions.JSONDecodeError:
                raw_snippet = response.text[:150] + '...'
                logger.warning(f"  Raw response (non-JSON): {raw_snippet}")
        else:
            error_snippet = response.text[:200]
            logger.error(f"  Error: {error_snippet}")

    except requests.exceptions.Timeout:
        elapsed = time.perf_counter() - start_time
        logger.error(f"  ❌ Timeout after {elapsed:.3f}s")
